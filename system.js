var sys = {
	...
	'Show' : function(oid) {
		var obj = $(oid);
		var lastObj = $('.t').filter(function(i){
			if(i == obj.index()){
				$('#send a').eq(i).addClass('ck');
				$('#head').html(document.title = $(this).data('title') || '字帖工厂');
			} else {
				$('#send a').eq(i).removeClass('ck');
			}
			return $(this).css('display') != "none";
		});
		if(obj.attr('id') !== lastObj.attr('id')) {
			lastObj.stop(true, false).fadeOut(200, function(){
				obj.fadeIn(200);
			});
		}
	},
